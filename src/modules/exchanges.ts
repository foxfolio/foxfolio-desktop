...
export default function reducer(
  ...
  switch (action.type) {
    ...
    case ExchangeTypeKeys.INCREMENT_EXCHANGE_REQUEST_COUNTER:
      return reduceInstance(
        state,
        action
      )(incrementExchangeRequestCounterInState);
  ...
  return async (dispatch: Dispatch, getState: GetState) => {
    ...
    await Promise.all(
      _.map(exchanges, (exchange) =>
        dispatch(fetchBalancesForExchange(exchange))
      )
    );
  };
}
...
function incrementExchangeRequestCounter(
  id: string
): IncrementExchangeRequestCounterAction {
  return {
    ...
    try {
      ...
      await dispatch(
        updateExchangeBalances(
          exchange.id,
          _.mapKeys(balances, (value, key) => unifySymbols(key))
        )
      );
    } catch (e) {
...
function fetchTradesForExchange(exchange: Exchange): ThunkAction {
  return async (dispatch: Dispatch) => {
    ...
    try {
      const connector: ccxt.Exchange = new ccxt[exchange.type](
...
// State Helpers
function addExchangeToState(
  state: Exchanges,
  action: AddExchangeAction
): Exchanges {
...
function deleteExchangeFromState(
  state: Exchanges,
  action: DeleteExchangeAction
): Exchanges {
...
function addFailedRequestToState(
  state: Exchange,
  action: FailedExchangeRequestAction
): Exchange {
...
type InstanceReducer<A extends ExchangeInstanceActions> = (
  state: Exchange,
  action: A
) => Exchange;
...
function reduceInstance<A extends ExchangeInstanceActions>(
  ...
  return (instanceReducer) => ({
    ...state,
    [action.id]: instanceReducer(state[action.id], action),
  });
}

function mergeArraysById<T extends { id: any }>(
  array: T[],
  newArray: T[]
): T[] {
  return _.unionWith(array, newArray, (a, b) =>
    _.isEqualWith(a, b, _.property("id"))
  );
}

const assign: <T>(a: T, ...b: Array<Partial<T>>) => T = (a, ...b) =>
  Object.assign({}, a, ...b);
